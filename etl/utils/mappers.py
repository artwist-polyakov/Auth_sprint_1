    ...
    def _map_dataclass(self, dataclass: FilmworkToTransform) -> DataclassForElastic:
        return DataclassForElastic(
            ...
            genres=[GenreElement(
                ...
                description=dataclass.genre_description if dataclass.genre_description else ''
            )],
            title=dataclass.title,
            description=dataclass.description,
            directors=[dataclass.person_full_name] if dataclass.role == 'director' else [],
            actor_names=[dataclass.person_full_name] if dataclass.role == 'actor' else [],
            writers_names=[dataclass.person_full_name] if dataclass.role == 'writer' else [],
            actors=[PersonIdName(person_id=str(dataclass.person_id),
                                 name=dataclass.person_full_name)]
            if dataclass.role == 'actor' else [],
            writers=[PersonIdName(person_id=str(dataclass.person_id),
                                  name=dataclass.person_full_name)]
            if dataclass.role == 'writer' else [],
            directors_list=[PersonIdName(person_id=str(dataclass.person_id),
                                         name=dataclass.person_full_name)]
            if dataclass.role == 'director' else []
        )

    def _match_diferences(
            self,
            current: DataclassForElastic,
            new: FilmworkToTransform
    ) -> DataclassForElastic:
        ...
        if new.role == 'actor':
            if PersonIdName(person_id=str(new.person_id),
                            name=new.person_full_name) not in current.actors:
                current.actors.append(PersonIdName(person_id=str(new.person_id),
                                                   name=new.person_full_name))
                current.actor_names.append(new.person_full_name)
                return current

        if new.role == 'director':
            if PersonIdName(person_id=str(new.person_id),
                            name=new.person_full_name) not in current.directors_list:
                current.directors_list.append(PersonIdName(person_id=str(new.person_id),
                                                           name=new.person_full_name))
        ...
        if new.role == 'writer':
            if PersonIdName(person_id=str(new.person_id),
                            name=new.person_full_name) not in current.writers:
                current.writers.append(PersonIdName(person_id=str(new.person_id),
                                                    name=new.person_full_name))
