Ссылка на приватный репозиторий с командной работой https://github.com/artwist-polyakov/Async_API_sprint_2

## Развёртывание проекта

1. Склонируйте репозиторий
2. Скачайте файл с настройками окружения [по ссылке](https://disk.yandex.ru/d/EqKF0k8QJgGKQA). Пароль `1234` 
3. В корневой папке проекта разместите файл `.env` с переменными окружения. Пример файла находится в `.env.example`.
4. Запустите проект командой `docker-compose up -d --build`
5. Приложение будет доступно по адресу http://localhost:8000/api/openapi
Необходмо согласимться с недостоверным сертификатом или добавить самоподписанный сертификат по инструкции в файле `FOR_TEAM.md`
